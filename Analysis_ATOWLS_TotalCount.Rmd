#Analysis Code for Atlantic NOS (Total Count)

ObservationCount = Total Count including the silent listening period. 

Data cleaning and manipulation script  can be found at DMEthier/National-NOS
...
out.dir<-"output/2019"
my.files<-list.files(path = out.dir, patter ="*ObservationCount.csv", full.name=TRUE)
...
```{r rawsummary, echo=FALSE, message=FALSE}

obs.sum<-obs %>% group_by(survey_year, CommonName) %>% summarise(count = sum(ObservationCount))
obs.sum<-cast(obs.sum, CommonName~survey_year, value="count")
...
#This was not fixed in underlying data. 
#if(obs$ProtocolCode=="37"){
#obs$StateProvince[obs$StateProvince  == "NS"]  <- "NB"  
#}
...
obs<-obs %>% select("CommonName", "species_id", "RouteIdentifier", "SiteCode", "collection", "ProtocolCode", "survey_year", "CollectorNumber", "ObservationCount")
...
grid.sum<-sp.data %>% group_by(survey_year, grid_id) %>% summarise(count = sum(ObservationCount))
grid.sum<-cast(grid.sum, grid_id~survey_year, value="count")  

##-----------------------------------------------------------
#zero fill by merging with the events dataframe. 
sp.data <- left_join(event.data, sp.data, by = c("RouteIdentifier", "survey_year", "CollectorNumber")) %>% mutate(ObservationCount = replace(ObservationCount, is.na(ObservationCount), 0))
...
write.table(sp.route.sum, paste(out.dir, "SpeciesRouteCountSummary2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
...
sp.data<-sp.data %>% group_by(RouteIdentifier, survey_year, CollectorNumber, grid_id, nstop, StateProvince, bcr, latitude, longitude) %>% summarise(count1=sum(ObservationCount))
...
write.csv(route_per_cell, paste(out.dir, species.list[m], ".route_per_cell2.csv", sep=""))
...
write.table(random.out, paste(out.dir, "Random_Summary2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
...
pdf(paste(out.dir, species.list[m], ".PITPlot2.pdf", sep=""))
...
write.table(d3, paste(out.dir, species.list[m], "AnnualIndex2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
...
write.table(tau_prov, paste(out.dir, "Tau_Prov2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
...
write.table(tau_tot, paste(out.dir, "Tau_Tot2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
...
write.csv(post_sum, paste(out.dir, species.list[m], "PosteriorSummary2.csv", sep="."))
...
pdf(paste(out.dir, species.list[m], ".MultiMapPlot2.pdf", sep=""))
multiplot(base1, alph_p1, tau_p2, route_p1, tau_p1, tau_p3, cols=2)
while(!is.null(dev.list())) dev.off()
...
write.table(new_prov, paste(out.dir, "Tau_Prov_Prec2.csv", sep="."), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)
