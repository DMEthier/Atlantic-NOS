...
##-----------------------------------------------------------
#Count the number of owls per route as the response varaible. The number of stop on a route can be used as a covarite in the model to control for route level effort
...
ATOWLS_Grid <- readOGR(dsn="C:/Users/dethier/Documents/ethier-scripts/Atlantic-NOS/Data", layer="ATOWLS_Grid")
names(ATOWLS_Grid)[1] <- "grid_id"; head(ATOWLS_Grid@data) #"+proj=aea +lat_0=40 +lon_0=-96 +lat_1=20 +lat_2=60 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs"
...
#  geom_histogram(breaks=c(0:150), fill="gray60", color="white") +
#  xlab("Number of routes per grid cell") + ylab("Count")

#summary(route_per_cell)

#print route_per_cell output table 
...
# map route
route1 <- as(ATOWLS_pt, "sf")
grid1 <- merge(grid1, route_per_cell, all=T)
grid1$number_routes[is.na(grid1$number_routes)] <- 0


route_p1 <- ggplot() +
  geom_sf(data=grid1, aes(fill=number_routes), col="gray40", size=0.3) +
...
# aggregate results 
# summarise at prov
new_prov <- post_sum %>% group_by(prov) %>%
